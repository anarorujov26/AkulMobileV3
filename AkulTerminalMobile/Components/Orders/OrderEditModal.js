...
const OrderEditModal = ({ route, navigation }) => {
      ...
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
        <View style={styles.container}>
          ...
          <View style={{ flex: 1, justifyContent: 'flex-end' }}>
            {
              type !== "ct" &&
              <CustomTextInput editable={pricePermission} addStyleInput={{ fontSize: 20, color: CustomColors("dark").connectedPrimary }} keyboardType={"numeric"} value={String(product.Discount)} onBlur={getDisPRI} text={'Endirim%'} width={'100%'} onChangeText={(e) => { setProduct(rel => ({ ...rel, ['Discount']: e.replace(',', '.') })) }} addStyle={{ borderRadius: 0, borderBottomWidth: 1 }} />
            }
