...
const ItemEditableComNew = ({ route, navigation }) => {

    const { data, setData, item } = route.params;
    const {setSaveButton} = useContext(ProductionsGlobalContext);
    ...
    const getInfo = async () => {
        ...
        let arrayData = [...data];
        let objectProduct = { ...item };

        let productAnswer = arrayData.findIndex(rel => rel.ProductId == objectProduct.Id);
        if (productAnswer != -1) {
            setProduct(arrayData[productAnswer]);
        } else {
            objectProduct.ProductId = objectProduct.Id;
    ...
    const getSaveInfo = async () => {
        ...
        let productAnswer = arrayData.findIndex(rel => rel.ProductId == thisProduct.ProductId);

        if (productAnswer != -1) {
            arrayData[productAnswer] = thisProduct;
        } else {
            arrayData.push(thisProduct);
        }
        setSaveButton(true)
        setData(arrayData);
        successAlert();
...
export default ItemEditableComNew
