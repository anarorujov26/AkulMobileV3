...
const Demand = ({ route, navigation }) => {
    ...
    const { saveButton, demand, setDemand, setSaveButton, setDemandListRender, setDebtQuantity } = useContext(DemandsGlobalContext)
    ...
    const getDemand = async (productId) => {
        if (productId == null) {
            ...
            const result = await Api('demandreturns/get.php', obj);
            ...
            data.Modifications = await modificationsGroup(result.data.Body.List[0], 'demandreturn');
    ...
    const getSaveProsessing = async () => {
        if (demand.CustomerId == "" || demand.StockId == "") {
            ...
            if (obj.name == "") {
                const result = await Api('demandreturns/newname.php', {
                    n: "",
                    token: await AsyncStorage.getItem("token")
                })
                if (result.data.Headers.ResponseStatus == "0") {
            ...
            const result = await Api('demandreturns/put.php', obj);
            if (result.data.Headers.ResponseStatus == "0") {
                setDemand(null)
                setSaveButton(false)
                successAlert();
