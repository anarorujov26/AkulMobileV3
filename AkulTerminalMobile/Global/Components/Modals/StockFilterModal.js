...
const StockFilterModal = ({ modalVisible, setModalVisible, idType, nameType, state, save }) => {

    const [group, setGroup] = useState([]);
    const [search, setSearch] = useState("");

    const getGroups = async () => {
        const result = await Api('stocks/get.php', { token: await AsyncStorage.getItem('token') })
        setGroup(result.data.Body.List)
    }

    const getSearchGroup = async () => {
        let obj = { name: search, token: await AsyncStorage.getItem('token') };
        const result = await Api('stocks/get.php', obj)
        setGroup(result.data.Body.List)
    }
